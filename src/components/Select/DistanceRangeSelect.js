...
class DistanceRangeSelect extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            isLoading: false,
    ...
    retrieveData(criteria = {}, inactive = {}, actionspage = 'create') {
        ...
        let sort = { distancerangename: 'asc' };
        ...
        RetrieveRequest(url, criteria, paging, column, sort).then((response) => {
            if (response.status.responsecode.substring(0, 1) === '0') {
                ...
                //if options deactive
                let { distancerangecode, distancerangename } = inactive;
                if (distancerangecode) {
                    options = getOptionsDeactive(actionspage, options, distancerangecode, distancerangename);
                }
