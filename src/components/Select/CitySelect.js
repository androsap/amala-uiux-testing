...
class CitySelect extends React.Component {
    ...
    retrieveData(criteria = {}, inactivefield = {}, actionspage = 'create') {
        ...
        RetrieveRequest(url, criteria, paging, column, sort).then((response) => {
            if (response.status.responsecode.substring(0, 1) === '0') {
                ...
                //if options deactive
                const { citycode, cityname } = inactivefield;
                if (citycode) {
                    options = getOptionsDeactive(actionspage, options, citycode, cityname);
                }
    ...
    getValue = (citycode = null, field = null) => {
        const { options } = this.state;
        let result = null;
        let detailoptions = options.filter(obj => obj.value === citycode)[0];
