...
class MembershipTypeSelect extends React.Component {
    ...
    retrieveData(criteria = {}, inactivefield = {}, actionspage = 'create') {
        ...
        let sort = { membershiptypename: 'asc' };
        ...
        RetrieveRequest(url, criteria, paging, column, sort).then((response) => {
            if (response.status.responsecode.substring(0, 1) === '0') {
                ...
                //if options deactive
                const { membershiptypeid, membershiptypename } = inactivefield;
                if (membershiptypeid) {
                    options = getOptionsDeactive(actionspage, options, membershiptypeid, membershiptypename);
                }
