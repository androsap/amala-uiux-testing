...
import { InputText, Button, Alert, TierSelect, BranchSelect, PartnerSelect, CustomTransactionSelect, SelectBase, DateRangeBase, SwitchButton } from '../../components/Base/BaseComponent';
...
class App extends Component {
    ...
    getDetail = (tierfirstactivitybonusid, actionspage) => {
        let url = api.url.firstactivitybonus.list;
        let criteria = { tierfirstactivitybonusid };
        //call loader
        ...
        RetrieveRequest(url, criteria).then((response) => {
            ...
            if (status.responsecode.substring(0, 1) === '0') {
                if (result.length !== 0) {
                    ...
                    let tiername = (result[0].tiername !== undefined) ? result[0].tiername : null;
    ...
    saveAction = (e) => {
        ...
        this.props.form.validateFieldsAndScroll((err, input) => {
            if (!err) {
                this.setState({ isLoading: true });
                //define parameter
                let tierid = (input.tierid) ? input.tierid : null;
                ...
                let data = { tierid, enrollchannel, activitystartperiod, activityendperiod, partnercode, branchcode, customtrxcode, tiermiles, awardmiles, frequency, isdefault };

                let message = '';
                let url = '';
                if (actionspage === 'create') {
                    message = 'New data has been created';
                    url = api.url.firstactivitybonus.create;
                } else {
                    message = 'Data has been updated';
                    url = api.url.firstactivitybonus.update;
                    data.tierfirstactivitybonusid = this.props.match.params.ID;
                }
                ...
                SaveRequest(url, data).then((response) => {
                    ...
                    if (responsecode.substring(0, 1) === '0') {
                        ...
                        this.props.history.push('/first-activity-bonus');
                    } else {
    ...
    render() {
        ...
        if (formrender) {
            ...
            document.title = titlepage + " First Activity Bonus | Loyalty Management System";
            //render form
            return (
                <Row>
                    <Row>
                        <Col xs={24} xl={22}>
                            <Title level={3}>{titlepage} First Activity Bonus</Title>
                        </Col>
                        <Divider />
                    </Row>
                    <Spin spinning={this.state.isLoading}>
                        <Form {...formItemLayout} onSubmit={this.saveAction}>
                            ...
                            <Row gutter={24} type="flex" justify="center" style={{ marginTop: 10 }}>
                                ...
                                <Button url="/first-activity-bonus" htmlType="link" type="default" label="Back" />
                            </Row>
                        </Form>
                    </Spin>
                </Row>
