...
import { Alert, Button, ErrorGeneral, InputText, CountrySelect } from '../../components/Base/BaseComponent';
...
class App extends Component {
    ...
    getDetail = (statecode, actionspage) => {
        ...
        RetrieveRequest(url, criteria).then((response) => {
            ...
            if (status.responsecode.substring(0, 1) === '0') {
                if (result.length !== 0) {
                    ...
                    let setValue = { statecode, statename, countrycode };
                    this.props.form.setFieldsValue(setValue);
                    let fieldvalue = { statecode, active };
        ...
        this.props.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                ...
                if (actionspage === 'create') {
                    ...
                    url = api.url.state.create;
                } else {
                    data.statecode = this.props.match.params.ID;
                    message = 'Data has been updated';
                    url = api.url.state.update;
                }
    ...
    deleteData(statecode, active) {
        let url = (active) ? api.url.state.deactivate : api.url.state.activate;
        let data = { statecode };
    ...
    render() {
        ...
        const { statecode, active } = this.state.fieldvalue;

        if (formrender) {
            document.title = titlepage + " State | Loyalty Management System";
            //render form
            return (
                <Row>
                    <Row>
                        <Col xs={24} xl={22}>
                            <Title level={3}>{titlepage} State</Title>
                        </Col>
                        <Divider />
                    </Row>
                        ...
                        <Form {...formItemLayout} onSubmit={this.saveAction}>
                            <Row gutter={24}>
                                <Col className="gutter-row" xs={24} sm={24} md={24} lg={{ span: 12, offset: 4 }} xl={{ span: 12, offset: 4 }}>
                                    <InputText labeltext="State Code" datafield="statecode" form={this.props.form} maxLength={5} validationrules={[`required`, `max.5`, `pattern.nospace`]} disabled={specialfielddisabled} />
                                    <InputText labeltext="State Name" datafield="statename" form={this.props.form} maxLength={45} validationrules={[`required`, `max.45`, `pattern.letterspace`]} disabled={generalfielddisabled} />
                            ...
                            <Row gutter={24} type="flex" justify="center" style={{ marginTop: 10 }}>
                                ...
                                }
                                {
                                    (actionspage !== 'create') ?
                                        (active) ?
                                            <Button htmlType="button" type="danger" label="Deactivate" menucode={menucode} prefixmenuname={prefixmenuname} actioncode="DELETE" onClick={() => this.deleteData(statecode, active)} /> :
                                            <Button htmlType="button" type="primary" label="Activate" menucode={menucode} prefixmenuname={prefixmenuname} actioncode="DELETE" onClick={() => this.deleteData(statecode, active)} /> : ""
                                }
                                <Button url="/state" htmlType="link" type="default" label="Back" />
                            </Row>
                        </Form>
                    </Spin>
                </Row>
