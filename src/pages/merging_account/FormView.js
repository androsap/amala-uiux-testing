...
class App extends React.Component {
        ...
        DetailRequest(url, criteria).then((response) => {
            ...
            if (status.responsecode.substring(0, 1) === '0') {
                if (result.length !== 0) {
                    ...
                    let isupdatable = (result.isupdatable) ? result.isupdatable : undefined;
                    let status = (result.status) ? result.status : undefined;

                    let setValue = { mergeid, requestnotes1, requestnotes2, requestnotes3, detailofrequest, remarks };
                    this.props.form.setFieldsValue(setValue);

                    let fieldvalue = { ...this.state.fieldvalue, isupdatable, status };
    ...
    getRetrieveOri = (cardnumbermastermember) => {
        ...
        // DetailRequest(url, { memberid: mastermember, type: 'ALL' }).then((response) => {
        //     const { status = {}, result } = response || {};
        //     if (status.responsecode === '0000') {
        //         if (response.result.status !== 'MERGED') {
    ...
    getRetrieveDes = (cardnumbermergewith) => {
        ...
        // DetailRequest(url, { memberid: mergewith, type: 'ALL' }).then((response) => {
        //     const { status = {}, result } = response || {};
        //     if (status.responsecode === '0000') {
        //         if (response.result.status !== 'MERGED') {
            ...
            () => {
                ...
                this.props.form.validateFieldsAndScroll((err, input) => {
                    if (!err) {
                        ...
                        let data = { mergeid, status, detailofrequest, requestnotes1, requestnotes2, requestnotes3, remarks, mastermember, mergewith }
    ...
    render() {
        const { isLoading, originPersonal, destinationPersonal, fieldvalue } = this.state;
        ...
        const { isupdatable, status } = fieldvalue;
                    ...
                    <Form>
                        <Row gutter={24}>
                            <Col className="gutter-row" xs={24} sm={24} md={24} lg={{ span: 12, offset: 4 }} xl={{ span: 12, offset: 6 }}>
                                ...
                                <TextArea form={this.props.form} labeltext="Remarks Process" datafield="remarks" maxLength={255} disabled={true} />
                            </Col>
                        </Row>
                    </Form>
                    {(isupdatable || status === 'REQUESTED') ?
                        <Row gutter={24} type='flex' justify='center' >
                            <Button htmlType="submit" type="danger" label="Cancel Request" onClick={() => this.getMergingAccount('CANCELED')} actioncode="UPDATE"></Button>
                        </Row>
                        : null
                    }
                </Row>
