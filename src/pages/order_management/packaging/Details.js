...
class App extends Component {
    ...
    componentDidMount() {
        document.title = 'Details Package | Loyalty Management System';
        this.getDetail();
    };

    getDetail = () => {
        let url = api.url.packaging.detail;
        let data = { orderpackedid: this.props.match.params.ID };
        let dataList = null;
        //call loader
        ...
        DetailRequest(url, data).then((response) => {
            ...
            if (status.responsecode === '0000') {
                if (result.length !== 0) {
                    let orderpackedid = result.orderpackedid ? result.orderpackedid : null;
                    ...
                    //destinationdetail
                    let address = result && result.destinationdetail && Object.keys(result.destinationdetail).length !== 0 && result.destinationdetail.address ? result.destinationdetail.address : null;
                    let branchname = result && result.destinationdetail && Object.keys(result.destinationdetail).length !== 0 && result.destinationdetail.branchname ? result.destinationdetail.branchname : null;
                    ...
                    let ticketofficename = result && result.destinationdetail && Object.keys(result.destinationdetail).length !== 0 && result.destinationdetail.ticketofficename ? result.destinationdetail.ticketofficename : null;
                    ...
                    this.setState({
                        orderpackedid, deadline, deadline2, reordernumber, isoversla, ordernotes, notes, status, startdate, enddate, name, ordercode, orderdate, mailingproductname, mailingproducttype,
                        ...
                        address, countryname, branchname, postalcode, statename, sendto, dataList, awb, addressvendor, phone, vendorcode, vendorname, email, usevendor, cityname, ticketofficename,
                        memberAddress, memberCountry, memberState, memberCity, memberPostalCode, lettername, languagename, papervariant, envelopevariant, papername, envelopename, mailingproductname,
    ...
    handleOk = () => {
        ...
        this.props.history.push({ pathname: `/order-management/package/`, state: {activeKey} });
    };
    ...
    render() {
        const { isLoading, deadline, isoversla, deadline2, reordernumber, status, ordernotes, startdate, enddate, name, mailingproducttype, mailingproductname, inventoryvariantname,
            cardnumber, enrolmentdate, tier, priorityhandling, visible, data, dataList, qty, address, countryname, branchname,
            postalcode, statename, sendto, usevendor, vendorname, VendorDetails, MemberDetails, visible2, cityname, ticketofficename,
            showAddModal, worknotesid, ordercode, orderdate, vendorcode, notes, lettername, languagename, papervariant, envelopevariant, papername, envelopename,
            memberAddress, memberCountry, memberState, memberCity, memberPostalCode, awb, activeKey } = this.state;
        ...
        return (
            <Row>
                <Row>
                    <Col xs={24} xl={22}>
                        <Title level={4}><Button2 htmlType={'link'} url={{ pathname: `/order-management/package/`, state: {activeKey} }} shape='circle' icon='left' /> Package Order Details</Title>
                    </Col>
                    <Divider />
                </Row>
                <Spin spinning={isLoading}>
                    ...
                    <Form {...formItemLayout}>
                                ...
                                <Card bordered={false} style={{ boxShadow: '0 1px 2.5px 0 rgba(27,27,27,.1)', marginBottom: '16px', borderRadius: '15px' }} >
                                        ...
                                        <Col xs={24} md={8} style={{ marginTop: 20 }}>
                                            ...
                                            <Col xs={21} offset={1}>
                                                ...
                                                <Row style={{ marginBottom: 5 }}>
                                                    ...
                                                    <Col className='gutter-row' xs={1}>: </Col>
                                                    <Col className='gutter-row' xs={23} md={16}> <label style={{ fontSize: '15px' }}>{(ticketofficename) ? ticketofficename : '-'}</label></Col>
                                                </Row>
                                                <Row style={{ marginBottom: 5 }}>
                                                    <Col className='gutter-row' xs={24} md={7}><Text type='secondary' style={{ fontSize: '13px' }}>Address</Text></Col>
                                                    <Col className='gutter-row' xs={1}>: </Col>
                                    ...
                                    <Row>
                                        <Col xs={24} md={8} style={{ marginTop: 20 }}>
                                            ...
                                            <Col xs={21} offset={1}>
                                                <Title level={4}>Packaging Info</Title>
                                                <Row style={{ marginBottom: 5 }}>
                                                ...
                                                <Row style={{ marginBottom: 5 }}>
                                                    <Col className='gutter-row' xs={24} md={7}><Text type='secondary' style={{ fontSize: '13px' }}>Package Vendor</Text></Col>
                                                    <Col className='gutter-row' xs={1}>: </Col>
                                        ...
                                        <Col xs={24} md={8} style={{ marginTop: 20 }}>
                                            ...
                                            <Col xs={21} offset={1} style={{ marginTop: 35 }}>
                                                ...
                                                <Row style={{ marginBottom: 5 }}>
                                                    ...
                                                    <Col className='gutter-row' xs={1}>: </Col>
                                                    <Col className='gutter-row' xs={23} md={16}> <label style={{ fontSize: '15px' }}>{status === 'READYTOPACK' ? 'READY TO PACK' : status}</label></Col>
                                                </Row>
                                                <Row style={{ marginBottom: 5 }}>
                        ...
                        <Row gutter={24} type="flex" justify="center" style={{ marginTop: 10 }}>
                            {
                                (status === 'READYTOPACK' && validateButton) ?
                                    <Button onClick={() => this.showModal()} type='primary' > Start Work </Button>
                                    : (status === 'PACKING' && validateButton) ?
                                        <Button onClick={() => this.showModal()} type='primary' > Finish Work </Button>
                                        : ''
                            }
                        </Row>
                        <Modal title={'Are you sure to update this data?'} visible={visible} onCancel={this.handleCancel} destroyOnClose={true} footer={null} width={800}>
