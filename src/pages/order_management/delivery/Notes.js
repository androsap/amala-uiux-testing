...
class App extends Component {
    ...
    componentDidMount() {
        this.props.form.setFieldsValue({ status: (this.props.data[0].status === 'READYTODELIVER') ? 'ON DELIVERY' : this.props.labeltext });
    ...
    saveAction = async (e) => {
        ...
        await this.props.form.validateFieldsAndScroll((err, input) => {
            if (!err) {
                ...
                let orderdeliveredids = this.props.data.map(val => val.orderdeliveredid);
                let notes = input.notes;
                let status = (this.props.data[0].status === 'READYTODELIVER') ? 'DELIVERING' : input.status;
                let startdate = (this.props.data[0].status === 'READYTODELIVER') ? moment(new Date()).format("YYYY-MM-DD") : null;
                let enddate = (this.props.data[0].status === 'DELIVERING') ? moment(new Date()).format("YYYY-MM-DD") : null;
                ...
                let data = { orderdeliveredids, notes, status, startdate, enddate, returnto, branchcode, ticketingofficecode };
                let message = 'New data has been updated';
                let url = api.url.delivery.bulk;
