...
class App extends React.Component {
    ...
    componentDidMount() {
        document.title = 'Manage Delivery Order | Loyalty Management System';
    ...
    showModal = (orderdeliveredid, type) => {
        if (type !== 'progress') this.setState({ orderdeliveredid, type, visible: true });
        if (type === 'progress') this.setState({ orderdeliveredid, type, visible2: true });
    };
    ...
    handleTabCliked = (key) => {
        ...
        let criteria = { orderdatestart: null, orderdateend: null, deadline: null, cardnumber: null, membername: null, ordercode: null, usevendor: null, mailingproducttype: null, mailingproductcode: null, inventoryvariantid: null, lettercode: null, priorityhandling: null, reordernumber: null };
        if (key === '1') {
            criteria.status = 'READYTODELIVER';
    ...
    downloadData = () => {
        const callback = () => {
            let orderdeliveredids = this.state.selectedRows.map(val => val.orderdeliveredid);
            let url = api.url.delivery.download
            let data = { orderdeliveredids: orderdeliveredids };
    ...
    render() {
        const { selectedRows, active, selectedRowKeys, expanded, visible, visible2, orderdeliveredid, type, visibleUpload, activeKey, placement, visibleNew, tab2clicked, tab3clicked, isLoading, visibleProgress, visibleFinish } = this.state;
        ...
        const configurationTable = {
            url: api.url.delivery.list,
            criteria: { status: activeKey === '2' ? 'DELIVERING' : activeKey === '3' ? '%U%' : 'READYTODELIVER' },
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: (value) => { return (value === 'READYTODELIVER') ? 'READY TO PICK-UP' : (value === 'DELIVERING') ? 'ON DELIVERY' : (value === 'SUCCESS') ? 'DELIVERED' : value }
                },
                {
                ...
                } : '',
                {
                    ...
                    render: (value, row) => {
                        return (
                            <span>
                                <Button url={{ pathname: `/order-management/delivery/details/` + encodeURIComponent(row.orderdeliveredid), state: { activeKey } }} size='small' title='Details' icon='eye' actioncode='UPDATE' />
                            </span>
                        )
                    }
                },
        ...
        const configSelectTable = {
            ...
            columns: [
                ...
                {
                    type: 'html', title: 'Deadline', dataIndex: 'deadline', sorter: false,
                    render: (value) => { return (value) ? moment(value).format('DD/MM/YYYY') : '-' }
                },
                {
            ...
            <React.Fragment>
                <Row>
                    <Col xs={24} sm={16} md={18} lg={20} xl={22}>
                        <Title level={3}>Manage Delivery Order List</Title>
                    </Col>
                ...
                <Tabs defaultActiveKey='1' style={{ marginTop: '-20px' }} onTabClick={this.handleTabCliked} activeKey={activeKey}>
                    <TabPane tab='NEW ORDER' key='1'>
                        <div>
                            <Drawer style={{ backgroundColor: '#9DDAF2' }} orderdeliveredid={orderdeliveredid} height={heightexpand} placement={placement} mask={false} closable={false} onClose={this.onCloseDrawwer} visible={visibleNew}
                                title={
                                    <Row gutter={24} >
                    ...
                    <TabPane tab='IN PROGRESS' key='2'>
                        <Col lg={24}></Col>
                        <Drawer style={{ backgroundColor: '#9DDAF2' }} orderdeliveredid={orderdeliveredid} height={heightexpand} placement={placement} mask={false} closable={false} onClose={this.onClose} visible={visibleProgress}
                            title={
                                <Row gutter={24} >
                                    <Col lg={8}><span style={{ marginLeft: 8 }}>
                                        {(selectedRows.length > 0) ? `Selected ${selectedRows.length} items` : ''}
                                    ...
                                    <Col lg={8}>
                                        <Row type='flex' justify='end'>
                                            <Button htmlType='button' size='small' label='DELIVERED/RETURN' type='primary' onClick={(e) => this.showModal(e, 'progress')} />
                                        </Row>
                                    </Col>
                                </Row>}
                        >
                        ...
                        <TableBase getCheckboxProps={record => ({
                            ...
                            children:
                                <span>
                                    ...
                                    {record.awb === null ?
                                        <span
                                            style={{ border: "4px solid red", marginLeft: 5 }} title={'AWB is null'}
                                        />
                                        : ''}
                                    {record.havemessage ?
                                        <span
                    ...
                    <TabPane tab='FINISH' key='3'>
                        <Drawer style={{ backgroundColor: '#9DDAF2' }} orderdeliveredid={orderdeliveredid} height={heightexpand} placement={placement} mask={false} closable={false} onClose={this.onClose} visible={visibleFinish}
                            title={
                                <Row gutter={24} >
                                    <Col lg={8}><span style={{ marginLeft: 8 }}>
                                        {(selectedRows.length > 0) ? `Selected ${selectedRows.length} items` : ''}
