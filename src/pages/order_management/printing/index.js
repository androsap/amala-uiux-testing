...
import { Form, Divider, Row, Col, Typography, Drawer, Table, Icon, Modal, Tooltip, Tag } from 'antd';
...
class App extends React.Component {
    ...
    componentDidMount() {
        document.title = 'Manage Printing Order | Loyalty Management System';
    ...
    showModal = (orderprintedid, type) => {
        if (type !== 'progress') this.setState({ orderprintedid, type, visible: true });
        if (type === 'progress') this.setState({ orderprintedid, type, visible2: true });
    };
    ...
    downloadData = () => {
        const callback = () => {
            let orderprintedids = this.state.selectedRows.map(val => val.orderprintedid);
            let url = api.url.printing.download
            let data = { orderprintedids: orderprintedids };
    ...
    render() {
        const { selectedRows, active, selectedRowKeys, expanded, visible, visible2, orderprintedid, type, visibleUpload, activeKey, placement, visibleNew, tab2clicked, tab3clicked, isLoading, visibleProgress, visibleFinish } = this.state;
        ...
        const configurationTable = {
            url: api.url.printing.list,
            criteria: { status: activeKey === '2' ? 'PRINTING' : activeKey === '3' ? 'PRINTED' : 'READYTOPRINT' },
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: (value) => { return (value === 'READYTOPRINT') ? 'READY TO PRINT' : value }
                },
                {
                ...
                } : '',
                {
                    ...
                    render: (value, row) => {
                        return (
                            <span>
                                <Button url={{ pathname: `/order-management/printing/details/` + encodeURIComponent(row.orderprintedid), state: { activeKey } }} size='small' title='Details' icon='eye' actioncode='UPDATE' />
                            </span>
                        )
                    }
                },
            ...
            <React.Fragment>
                <Row>
                    <Col xs={24} sm={16} md={18} lg={20} xl={22}>
                        <Title level={3}>Manage Printing Order List</Title>
                    </Col>
                ...
                <Tabs defaultActiveKey='1' style={{ marginTop: '-20px' }} onTabClick={this.handleTabCliked} activeKey={activeKey}>
                    <TabPane tab='NEW ORDER' key='1'>
                        <div>
                            <Drawer style={{ backgroundColor: '#9DDAF2' }} orderprintedid={orderprintedid} height={heightexpand} placement={placement} mask={false} closable={false} onClose={this.onCloseDrawwer} visible={visibleNew}
                                title={
                                    <Row gutter={24} >
                    ...
                    <TabPane tab='IN PROGRESS' key='2'>
                        <Col lg={24}></Col>
                        <Drawer style={{ backgroundColor: '#9DDAF2' }} orderprintedid={orderprintedid} height={heightexpand} placement={placement} mask={false} closable={false} onClose={this.onClose} visible={visibleProgress}
                            title={
                                <Row gutter={24} >
                                    <Col lg={8}><span style={{ marginLeft: 8 }}>
                                        {(selectedRows.length > 0) ? `Selected ${selectedRows.length} items` : ''}
                    ...
                    <TabPane tab='FINISH' key='3'>
                        <Drawer style={{ backgroundColor: '#9DDAF2' }} orderprintedid={orderprintedid} height={heightexpand} placement={placement} mask={false} closable={false} onClose={this.onClose} visible={visibleFinish}
                            title={
                                <Row gutter={24} >
                                    <Col lg={8}><span style={{ marginLeft: 8 }}>
                                        {(selectedRows.length > 0) ? `Selected ${selectedRows.length} items` : ''}
