...
class App extends Component {
    ...
    componentDidMount() {
        this.props.form.setFieldsValue({ status: (this.props.data[0].status === 'READYTOPRINT') ? 'PRINTING' : this.props.labeltext });
    }
    ...
    saveAction = async (e) => {
        ...
        await this.props.form.validateFieldsAndScroll((err, input) => {
            if (!err) {
                ...
                //define parameter
                let orderprintedids = this.props.data.map(val => val.orderprintedid);
                let notes = input.notes;
                let status = (this.props.data[0].status === 'READYTOPRINT') ? 'PRINTING' : 'PRINTED';
                let usevendor = this.props.data[0].usevendor;
                let reordernumber = this.props.data[0].reordernumber;
                let startdate = (this.props.data[0].status === 'READYTOPRINT') ? moment(new Date()).format("YYYY-MM-DD") : null;
                let enddate = (this.props.data[0].status === 'PRINTING') ? moment(new Date()).format("YYYY-MM-DD") : null;

                let data = { orderprintedids, notes, status, usevendor, reordernumber, startdate, enddate };
                let message = 'New data has been updated';
                let url = api.url.printing.bulk;
                    ...
                    <Form {...formItemLayout} onSubmit={this.saveAction}>
                        <Row gutter={24}>
                            <Col className="gutter-row" xs={24} sm={24} md={24} lg={{ span: 22, offset: 0 }} xl={{ span: 22, offset: 0 }}>
                                <TextArea form={this.props.form} labeltext={(this.props.type === 'finish') ? 'Notes for Packaging Vendor' : 'Notes For Vendor/GA'} datafield="notes" validationrules={['required']} disabled={generalfielddisabled} maxRows={7} />
                            </Col>
