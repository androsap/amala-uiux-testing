...
class App extends Component {
    ...
    componentDidMount() {
        document.title = 'Details Printing | Loyalty Management System';
        this.getDetail();
    };

    getDetail = () => {
        let url = api.url.printing.detail;
        let data = { orderprintedid: this.props.match.params.ID };
        let dataList = null;
        //call loader
        ...
        DetailRequest(url, data).then((response) => {
            ...
            if (status.responsecode === '0000') {
                if (result.length !== 0) {
                    let orderprintedid = result.orderprintedid ? result.orderprintedid : null;
                    ...
                    this.setState({
                        orderprintedid, deadline, deadline2, reordernumber, isoversla, ordernotes, notes, status, startdate, enddate, name, ordercode, orderdate, mailingproductname, mailingproducttype,
    ...
    handleOk = () => {
        ...
        this.props.history.push({ pathname: `/order-management/printing/`, state: {activeKey} });
    };
    ...
    render() {
        const { isLoading, deadline, isoversla, deadline2, reordernumber, status, ordernotes, startdate, enddate, name, mailingproducttype, inventoryvariantname,
            ...
            memberAddress, memberCountry, memberState, memberCity, memberPostalCode, mailingproductname, awb, activeKey } = this.state;
        ...
        return (
            <Row>
                <Row>
                    <Col xs={24} xl={22}>
                        <Title level={4}><Button2 htmlType={'link'} url={{ pathname: `/order-management/printing/`, state: {activeKey} }} shape='circle' icon='left' /> Printing Order Details</Title>
                    </Col>
                    <Divider />
                </Row>
                <Spin spinning={isLoading}>
                    ...
                    <Form {...formItemLayout}>
                                ...
                                <Card bordered={false} style={{ boxShadow: '0 1px 2.5px 0 rgba(27,27,27,.1)', marginBottom: '16px', borderRadius: '15px' }} >
                                    ...
                                    <Row>
                                        <Col xs={24} md={8} style={{ marginTop: 20 }}>
                                            ...
                                            <Col xs={21} offset={1}>
                                                <Title level={4}>Printing Info</Title>
                                                <Row style={{ marginBottom: 5 }}>
                                                ...
                                                <Row style={{ marginBottom: 5 }}>
                                                    <Col className='gutter-row' xs={24} md={7}><Text type='secondary' style={{ fontSize: '13px' }}>Printing Vendor</Text></Col>
                                                    <Col className='gutter-row' xs={1}>: </Col>
                                        ...
                                        <Col xs={24} md={8} style={{ marginTop: 20 }}>
                                            ...
                                            <Col xs={21} offset={1} style={{ marginTop: 35 }}>
                                                ...
                                                <Row style={{ marginBottom: 5 }}>
                                                    ...
                                                    <Col className='gutter-row' xs={1}>: </Col>
                                                    <Col className='gutter-row' xs={23} md={16}> <label style={{ fontSize: '15px' }}>{status === 'READYTOPRINT' ? 'READY TO PRINT' : status}</label></Col>
                                                </Row>
                                                <Row style={{ marginBottom: 5 }}>
                        ...
                        <Row gutter={24} type="flex" justify="center" style={{ marginTop: 10 }}>
                            {
                                (status === 'READYTOPRINT' && validateButton) ?
                                    <Button onClick={() => this.showModal()} type='primary' > Start Work </Button>
                                    : (status === 'PRINTING' && validateButton) ?
