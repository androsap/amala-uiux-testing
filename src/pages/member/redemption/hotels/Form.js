...
import { SalutationSelect, Alert, SelectBase, SwitchButton, InputText, Button, DateRangeBase, ErrorGeneral, NomineeCardNumberSelect } from '../../../../components/Base/BaseComponent';
import { Form, Row, Col, Divider, Typography, Spin, Modal, Affix, Card, Tooltip, Alert as AlertAnt } from 'antd';
...
class App extends Component {
    ...
    componentDidMount() {
        ...
        document.title = "Redemption Hotel | Loyalty Management System";
    ...
    render() {
        ...
        const { isSuccessBuy, responseBuyAward, formrender, awardinfo, pricingby, promocode,
            quantity, visible, isLoading, cardnumber, totalafterdiscount, discount, discounttype, totaldiscount } = this.state;

        // let codepromo = this.props.form.getFieldValue('codepromo') ? this.props.form.getFieldValue('codepromo') : null;
        ...
        if (isSuccessBuy) {
            ...
            return (
                <Row>
                    ...
                    <Row>
                        <Title level={4}><Button url={'/member/form/' + memberid + '/redemption'} shape="circle" icon="left" />  Redemption Hotel</Title>
                        <Divider />
                    </Row>
                                            ...
                                            <Card title="Price Details" bordered={false} style={{ boxShadow: '0 2px 5px 0 rgba(27,27,27,.1)', marginTop: '10px' }}>
                                                <Row>
                                                    ...
                                                    {/* <Col xs={24} sm={24} md={24} lg={8} xl={8} style={{ textAlign: 'right' }}>
                                                        {(totalMileage && totalafterdiscount !== null) ? <Text delete type="secondary">{formatNumber(totalMileage)}</Text> : (totalMileage && totalafterdiscount == null) ? formatNumber(totalMileage) : '-'}
                                                    </Col> */}
                                                    {/* <Col xs={24} sm={24} md={24} lg={8} xl={8} style={{ textAlign: 'right' }}>
                                                        {(price && totalafterdiscount !== null) ? <Text strong>{formatNumber(totalafterdiscount * quantity)}</Text> : null}
                                                    </Col> */}
                                                    {/* {
                                                        (totalafterdiscount !== null) ?
                                                            <div>
