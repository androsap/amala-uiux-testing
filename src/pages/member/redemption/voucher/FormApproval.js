...
import { SalutationSelect, Button, Alert, SelectBase, SwitchButton, InputText, InputNumber, ErrorGeneral } from '../../../../components/Base/BaseComponent';
import { Form, Row, Col, Divider, Typography, Spin, Modal, Card, Affix, Button as AntButton, Tooltip, Alert as AlertAnt } from 'antd';
...
class App extends Component {
    ...
    componentDidMount() {
        const { redeemuser, quantity } = this.props.result.reqdatas;
        const { certificateprice, familyname, memberiduser, name, selfusage, travelertype } = redeemuser[0] || redeemuser;
        this.props.form.setFieldsValue({ certificateprice, [`passenger-familyname0`]: familyname, [`passenger-memberid0`]: memberiduser, [`passenger-name0`]: name, selfusage, [`passenger-travelertype0`]: travelertype, quantity });
    ...
    saveAction = (e) => {
        ...
        const callback = (input) => {
            ...
            const { referenceid, requestid, requeststatus, requesttype, memberid, approvalby, approvaldate, reqdata, createdBy, isdataactive, reqdatas } = this.props.result;
            ...
            let data = {
                requestid, referenceid, memberid, requesttype, requeststatus, approvalby, approvaldate, remark: input.remark ? input.remark : null, reqdata, createdBy, isdataactive, reqdatas: {
                    url: 'redemption/transaction/v1.2/buyaward', promocode, channelapp, awardcode, issueddate, quantity, memberid, username, categorycode,
                    bookingcode, ticketnumber, ticketvaliditydate, freeaward, totalprice, redeemuser, redeemairactivity, remark: input.remark ? input.remark : null, return: null
                }
            };
    ...
    render() {
        ...
        const { isSuccessBuy, showsave, formrender, awardinfo, pricingby, promocode, visible, isLoading, cardnumber, totalafterdiscount, discount, discounttype, totaldiscount } = this.state;
                                ...
                                <Col className="gutter-row" xs={24} sm={24} md={24} lg={16} xl={16}>
                                    ...
                                    <Row>
                                        ...
                                        {(promocode) ?
                                            ...
                                            : null}
                                        <Col className="gutter-row" xs={24} sm={24} md={24} lg={{ span: 24, offset: 2 }} xl={{ span: 20, offset: 2 }}>
                                            <Col xs={24} lg={14}>
                                            ...
                                            <Col xs={24} lg={10}>
                                                <Button htmlType="button" size="medium" label="Promo List" onClick={(e) => this.handleOpenModal(e)} style={{ marginLeft: 5, marginTop: 3 }} />
                                            </Col>
                                        </Col>
                                        <Col className="gutter-row" xs={24} sm={24} md={24} lg={{ span: 20, offset: 2 }} xl={{ span: 20, offset: 2 }}>
