...
import TrackOrder from './Form/TrackOrder';
...
class App extends React.Component {
    ...
    componentDidMount() {
        document.title = 'Tracking Status | Loyalty Management System';
        this.getList();
    };
    ...
    getList = async () => {
        ...
        await RetrieveRequest(api.url.memberbuyproduct.retrieve, { ordercode }).then(async (response) => {
            ...
            if (responsecode === '0000' && result && result.length !== 0) {
                const { sendto, country, state, city, mailingproduct, lettercode, vendorregion } = result[0]

                await DetailRequest(api.url.mailingproduct.detailintegration, { mailingproductcode: mailingproduct.mailingproductcode }).then(async (response) => {
                    const { status, result } = response;
                    const { responsecode, responsemessage } = status;
        ...
        await DetailRequest(api.url.member.profile, { memberid: this.props.match.params.ID, type: 'SUMMARY' }).then(async (response) => {
            const { status, result } = response;
            const { responsecode, responsemessage } = status;
            if (responsecode === '0000' && result) {
    ...
    render() {
        ...
        return (
            <React.Fragment >
                <TrackOrder {...this.props} isLoading={isLoading} trackingDetail={trackingDetail} orderDetail={orderDetail} memberProfile={memberProfile} letterData={letterData} vendorProduct={vendorProduct} />
            </React.Fragment >
        );
    }
}
