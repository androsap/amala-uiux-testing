...
import { Form, Row, Col, Divider, Typography, Spin, Modal, Card, Affix, Tooltip, Alert as AlertAnt, Statistic, Tag } from 'antd';
import { formatNumber, getTravelerType, FinishOTP } from '../../../../utilities/Helpers';
...
class App extends Component {
    ...
    componentDidMount() {
        document.title = 'Redemption OTP Voucher | Loyalty Management System';
    ...
    saveAction = (e, typeButton) => {
        ...
        const callback = (input) => {
            ...
            redeemuser.push({
                ...
                certificateprice: (totalafterdiscount !== null) ? totalafterdiscount : Number(price),
                activitydate: null
            });
    ...
    handlePriceBy = (rules, value, callback) => {
        let price = (this.state.pricingby === 'FIXED') ? this.state.price : ((value < 0) || (value > 99999999999) || (value.match(/^[0-9]+$/) === null)) ? 0 : value
    ...
    render() {
        ...
        let quantity = this.props.form.getFieldValue('quantity') ? this.props.form.getFieldValue('quantity') : 0;
        ...
        if (statusScreenOTP === 'allowed') {
            ...
            if (isSuccessBuy) {
                return (<Redirect to={{ pathname: `/member/form/${memberid}/redemptionotp/voucher/${awardcode}/certificate`, state: { responseBuyAward } }} />)
            } else {
                return (
                    <Row>
                        ...
                        <Row>
                            <Col xs={(xtraSmallWidthScreen) ? 18 : 12}>
                                <Title level={4}> <Button url={{ pathname: `/member/form/${memberid}/redemptionotp` }} shape='circle' icon='left' />  {(xtraSmallWidthScreen) ? 'Redeem OTP Voucher' : 'Redemption with OTP Voucher'}</Title>
                            </Col>
                                    ...
                                    <Col className='gutter-row' xs={24} sm={24} md={24} lg={16} xl={16}>
                                        ...
                                        <Row>
                                            ...
                                            <Col className='gutter-row' xs={24}>
                                                ...
                                                {(pricingby === 'MANUAL') ? null : <Row gutter={24}>
                                                    <Col xs={14}>
                                                    ...
                                                    <Col xs={8}>
                                                        <Button htmlType='button' size='medium' label='Promo List' onClick={(e) => this.handleOpenModal(e)} disabled={(quantity) ? false : true} style={{ marginTop: (xtraSmallWidthScreen) ? 32 : 3 }} />
                                                    </Col>
                                                </Row>}
                                            </Col>
                                        </Row>
