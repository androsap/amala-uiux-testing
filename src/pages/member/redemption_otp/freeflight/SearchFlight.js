...
import { Form, Row, Col, Divider, Typography, Spin, Statistic, Tag, Tooltip } from 'antd';
...
class App extends Component {
    ...
    handleCompartmentChange = (compartmentcode) => {
        ...
        if (compartmentcode) this.componentSubclassSelect.retrieveData({ airlinecode, compartmentcode, spendmiles: true });
    ...
    render() {
        ...
        if (statusScreenOTP === 'allowed') {
            ...
            if (isSuccessSearchFlight) {
                return (<Redirect to={{ pathname: `${this.props.match.url}/buy`, state: { priceList: responseSearchFlight, requestSearchFlight: requestSearchFlight, awardinfo } }} />)
            } else {
                return (
                    <Row>
                        <Row>
                            <Col xs={(xtraSmallWidthScreen) ? 18 : 12}>
                                <Title level={4}> <Button url={{ pathname: `/member/form/${this.props.match.params.ID}/redemptionotp` }} shape='circle' icon='left' />  {(xtraSmallWidthScreen) ? 'Redeem OTP Freeflight' : 'Redemption with OTP Freeflight'}</Title>
                            </Col>
