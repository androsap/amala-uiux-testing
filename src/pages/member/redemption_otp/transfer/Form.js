...
import { Form, Row, Col, Divider, Typography, Spin, Card, Statistic, Tooltip, Tag } from 'antd';
...
class App extends Component {
    ...
    componentDidMount() {
        document.title = ' Redemption OTP Transfer | Loyalty Management System ';
    ...
    render() {
        ...
        const { otpsessionid, countdownSession, statusScreenOTP } = this.props.dataOTP;
        ...
        if (statusScreenOTP === 'allowed') {
            if (isSuccessBuy) {
                return (<Redirect to={{ pathname: `/member/form/${memberid}/redemptionotp/transfer/${awardinfo.awardcode}/certificate`, state: { responseBuyAward } }} />)
            } else {
                if (formrender) {
                    return (
                        <Row>
                            <Row>
                                <Col xs={(xtraSmallWidthScreen) ? 18 : 12}>
                                    <Title level={4}> <Button url={{ pathname: `/member/form/${memberid}/redemptionotp` }} shape='circle' icon='left' />  {(xtraSmallWidthScreen) ? 'Redeem OTP Transfer' : 'Redemption with OTP Transfer'}</Title>
                                </Col>
                                ...
                                <Form {...formItemLayout} onSubmit={this.saveAction}>
                                    <Row gutter={24}>
                                        ...
                                        <Col className={'gutter-row'} xs={24} sm={24} md={24} lg={10} xl={11} >
                                            <Card title='Recipient' bordered={false} className='card-shadow' style={{ marginBottom: 30 }}>
                                                ...
                                                <Row style={{ marginTop: 10 }}>
                                                    ...
                                                    <Col xs={24} sm={24} md={24} lg={14} xl={14} style={{ textAlign: 'right' }}>
                                                        {awardinfo.cardnumber}
                                                        {/* - {awardinfo.nameoncard === null || awardinfo.nameoncard === undefined ? `${awardinfo.firstname} ${awardinfo.lastname}` : awardinfo.nameoncard} */}
                                                    </Col>
                                                </Row>
                                                <Row style={{ marginTop: 10 }}>
                                            ...
                                            <Button htmlType='button' type='primary' block={true} label='Send' menucode={'REDEEMOTP'} prefixmenuname={'REDOTP'} actioncode={'BUY'} style={{ marginTop: '20px' }}
                                                disabled={awardinfo.pricingby === 'FIXED' ? (quantity && trxdate ? false : true) : (price && trxdate ? false : true)} onClick={(e) => this.saveAction(e, 'BUY')} />
                                            <Button htmlType='button' type='primary' block={true} label='Request' menucode={'REDEEMOTP'} prefixmenuname={'REDOTP'} actioncode={'REQUEST'} style={{ marginTop: '20px' }}
                ...
                } else return (<ErrorGeneral {...this.props} message={errormessage} />);
            };
        } else return (
            <Row>
