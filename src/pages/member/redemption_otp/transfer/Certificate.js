    ...
    render() {
        ...
        if (statusScreenOTP === 'allowed') {
            return (
                <Row>
                    <Col xs={(xtraSmallWidthScreen) ? 18 : 12}>
                        <Title level={4}> <ButtonComponent url={{ pathname: `/member/form/${memberid}/redemptionotp` }} shape='circle' icon='left' />  {(xtraSmallWidthScreen) ? 'Certif OTP Transfer' : 'Certificate with OTP Transfer'}</Title>
                    </Col>
            ...
            <Row>
                ...
                <Row gutter={24} type='flex' justify='center'>
                    <ButtonComponent url={`/member/form/${memberid}/redemptionotp`} htmlType='link' type='default' label='Back' />
                </Row>
            </Row>
        )
    };
