...
import { Card, Row, Col, Modal, Spin, Form, Typography } from 'antd';
...
class App extends React.Component {
    ...
    render() {
        const { certificateid, confirmationtype, memberid, awardcategory, certificatedetails, typeButton, cancelunit, cancelfee, feeinfo, redeemairactivity, selectedActivities } = this.props;
        ...
        let percentagefee = this.props.percentagefee;

        if (certificatedetails && certificatedetails.awardcode === 'FREEFLIGHT') {
            let totalFee = selectedActivities.map((val, i) => {
                let airactivityid = val.split('|SPLIT|')[0];
                let selectedAirActivities = redeemairactivity.find((val) => val.redeemairactivityid === airactivityid);
                let flightfee = selectedAirActivities.price;

                if (selectedAirActivities.userefundfee) {
                    if (cancelunit === 'PERCENTAGE') {
                        return Math.ceil(flightfee * (cancelfee / 100));
                    } else return cancelfee;
                } else return 0;
            });
            percentagefee = totalFee.reduce((a, b) => a + b, 0);
        };
