...
class App extends Component {
    ...
    async componentDidMount() {
        ...
        let promocodedepr = flightdeparture ? flightdeparture.promocode : null;
        let promocoderetr = flightreturn ? flightreturn.promocode : null;
        /*Variable props */
    ...
    saveAction = (e, callbackexpiration) => {
        ...
        const callback = (input) => {
            ...
            const { fromApproval, result, profile } = this.props;
            ...
            const { certificatedetails, activitydetails, activitydepartureselected, activityreturnselected, requestSearchFlight } = state;
            const { adultpassenger } = requestSearchFlight;
            const { certificateid, redeemusers, status } = certificatedetails;
            const { roundtrip } = activitydetails;
            ...
            let trxdate = moment(certificatedetails.issueddate)._i.replaceAll('/', '-');
            ...
            let data = (fromApproval) ? {
                ...
                certificateid, issueddate, memberid, bookingcode, totalprice, updatetype, redeemairactivity, standarfee, fee, trxdate, cardnumber: profile.cardnumber
            } : {
                referenceid: null, memberid: memberid, requesttype: "UPDATE", requeststatus: "NEW", approvalby: null, approvaldate: null, remark: null, reqdatas: {
                    status, certificateid, issueddate, memberid, bookingcode, totalprice, updatetype, redeemairactivity, standarfee, fee, trxdate, cardnumber: profile.cardnumber,
                    url: 'redemption/transaction/v1.2/change',
                }
            }
    ...
    showConfirmation = (typeButton) => {
        ...
        if (!roundtrip && promocodedepr && activitydepartureselected) {
            ...
            this.getpromoConfirm('RETURN', true, false)
        } else if (roundtrip && !promocoderetr && promocodedepr && activitydepartureselected) {
            this.getpromoConfirm('DEPARTURE', true, false)
        } else if (roundtrip && promocoderetr && promocodedepr && activitydepartureselected) {
            this.getpromoConfirm('DEPARTURE', true, false)
    ...
    render() {
        ...
        const { isLoading, flightDeparture, flightReturn, selectFlightDeparture, selectFlightReturn, requestSearchFlight, mileageDeparture, mileageReturn, isSuccessBuy, newcertificateid, deprpromocode, discountamountdepr, deprprice, retrpromocode,
            discountamountretr, retrprice, typeButton, discountdepr, totaldiscountdepr, totalafterdiscountdepr, promocodedepr, discounttypedepr, discountretr, totaldiscountretr,
            totalafterdiscountretr, promocoderetr, discounttyperetr, showsave, fieldvalue, showConfirmation, visible } = this.state;
        const { adultpassenger } = requestSearchFlight;
        const { location, permission, fromApproval, result } = this.props;
        const { state } = location;
        ...
        const { updateunit, updatefee } = fieldvalue || {};
        ...
        let changeFee = (fee) ? `${(updateunit === 'PERCENTAGE') ? ((100 * fee) / updatefee) : fee}` : '';
        ...
        let totalmileage = (totalafterdiscountdepr !== null && totalafterdiscountretr === null && fee !== undefined) ? formatNumber(Math.ceil((totalafterdiscountdepr + mileageReturn + parseInt(fee, 0)) * adultpassenger)) :
        ...
        let informationDetail = <div style={{ overflowY: "auto", height: "100vh", padding: '2vh' }}>
            ...
            <Card title="Price Details" bordered={false} style={{ boxShadow: '0 2px 5px 0 rgba(27,27,27,.1)', marginTop: '10px' }} bodyStyle={{ paddingTop: '5px', paddingBottom: '5px' }}>
                <Row>
                    ...
                    <Col xs={24} sm={24} md={24} lg={6} xl={6} style={{ textAlign: 'right', marginTop: 53 }}>
                    ...
                    <Col xs={24} sm={24} md={24} lg={18} xl={18} style={{ display: 'block', marginLeft: 30 }}>
                        <Text style={{ display: 'block' }}>
                            {(discountdepr && discounttypedepr === "PERCENTAGE" && totaldiscountdepr) ? `(Disc ${discountdepr}%) @${formatNumber(totaldiscountdepr)} x ${adultpassenger}` :
                                (!discountdepr && deprpromocode && discountamountdepr) ? /*`(Disc ${discountdepr}%)*/ `Disc @${formatNumber(discountamountdepr)} x ${adultpassenger}` :
                ...
                </Row>
                {
                    (roundtrip) ?
                        <Row style={{ marginTop: '10px' }}>
                            <Col xs={24} sm={24} md={24} lg={18} xl={18}>
                                <Text style={{ display: 'block' }}> Return Flight </Text>
                            ...
                            <Col xs={24} sm={24} md={24} lg={18} xl={18} style={{ display: 'block', marginLeft: 30 }}>
                                <Text style={{ display: 'block' }}>
                                    {(discountretr && discounttyperetr === "PERCENTAGE" && totaldiscountretr) ? `(Disc ${discountretr} %) @${formatNumber(totaldiscountretr)} x ${adultpassenger}` :
                                        (!discountretr && retrpromocode && discountamountretr) ? /*`(Disc ${discountdepr}%)*/ `Disc @${formatNumber(discountamountretr)} x ${adultpassenger}` :
                ...
                <Row style={{ marginTop: '12px' }}>
                    <Col xs={24} sm={24} md={24} lg={18} xl={18}>
                        <Text strong style={{ display: 'block' }}>Change Fee {(updateunit && updatefee) ? `(${updatefee}${(updateunit === 'PERCENTAGE') ? `% x ${formatNumber(changeFee)}` : ` Mileage`})` : ''}</Text>
                    </Col>
                    <Col xs={24} sm={24} md={24} lg={6} xl={6} style={{ textAlign: 'right' }}>
        ...
        if (isSuccessBuy) {
            ...
            return (
                <Row>
                    {
                        (showConfirmation) ? <ConfirmationPage isLoading={isLoading} certificateid={certificateid} memberid={memberid} departureFlight={fieldvalue.summaryDepartureFlight} returnFligth={fieldvalue.summaryReturnFlight} roundtrip={roundtrip} visible={showConfirmation} handleClose={this.handleCancel} fee={fee} totalmileage={totalmileage}
                            onSubmit={this.saveAction} handleShowNewCertificate={this.handleShowNewCertificate} location={location} mileageDeparture={mileageDeparture} mileageReturn={mileageReturn} discountamountdepr={discountamountdepr} discountamountretr={discountamountretr} typeButton={typeButton} standarfee={standarfee} /> : null
                    }
                                ...
                                <Col className="gutter-row" xs={24} sm={24} md={24} lg={15} xl={16}>
                                    <Row>
                                        ...
                                        <Col className="gutter-row" xs={24} sm={24} md={24} lg={24} xl={24}>
                                            ...
                                            <Row className="pricelist-flight-header">
                                                ...
                                                <Col xs={24} sm={24} md={24} lg={6} xl={6}>Compartment /<br />Booking Class</Col>
                                                <Col xs={24} sm={24} md={24} lg={5} xl={5}>Price</Col>
                                            </Row>
                                            <List
                                                ...
                                                renderItem={(item) => (
                                                    <Row className="pricelist-flight">
                                                        ...
                                                        <Col xs={24} sm={24} md={24} lg={5} xl={5}>{(item.origin) ? item.origin : '-'} - {(item.destination) ? item.destination : '-'}</Col>
                                                        <Col xs={24} sm={24} md={24} lg={6} xl={6}>
                                                            {(item.compartmentcode) ? item.compartmentcode : '-'}&nbsp;/&nbsp;
                                                            {(item.bookingclasscode) ? item.bookingclasscode : '-'}
                                                        </Col>
                                                        <Col xs={24} sm={24} md={24} lg={5} xl={5}>{(totalafterdiscountdepr) ? formatNumber(totalafterdiscountdepr) : (item.price) ? formatNumber(item.price) : '-'}</Col>
                                                        {
                                    ...
                                    <Row className={((!roundtrip && selectFlightDeparture !== null) || (roundtrip && selectFlightDeparture !== null && selectFlightReturn !== null)) ? '' : 'hidden'}>
                                        ...
                                        <Col className="gutter-row" xs={24} sm={24} md={24} lg={{ span: 23, offset: 1 }} xl={{ span: 20, offset: 2 }}>
                                            ...
                                            <InputText form={this.props.form} labeltext="Departure Flight Number" datafield="flightnumberdeparture" validationrules={['required', 'pattern.number']} maxLength={4} disabled={!activitydepartureselected} onChange={() => this.handleFlightNumber(flightnumberdeparture, "DEPARTURE")} />
                                            {
                                                (roundtrip) ? <span>
                                                    ...
                                                    <InputText form={this.props.form} labeltext="Return Flight Number" datafield="flightnumberreturn" validationrules={['required', 'pattern.number']} maxLength={4} disabled={!activityreturnselected} onChange={() => this.handleFlightNumber(flightnumberreturn, "RETURN")} />
                                                </span> : null
                                            }
                                        </Col>
                                    </Row>
                                    ...
                                    <Row className={((!roundtrip && selectFlightDeparture !== null) || (roundtrip && selectFlightDeparture !== null && selectFlightReturn !== null)) ? '' : 'hidden'}>
                                        ...
                                        <Col className="gutter-row" xs={24} sm={24} md={24} lg={{ span: 23, offset: 1 }} xl={{ span: 20, offset: 2 }}>
                                            ...
                                            {
                                                (standarfee) ? <Form.Item label="Fee">
