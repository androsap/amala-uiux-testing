...
class App extends Component {
    ...
    getDetail = () => {
        ...
        if (fromApproval) {
            ...
            activitydepartureselected = (flightdeparture) ? true : false;
            activityreturnselected = (flightreturn) ? true : false;
                        ...
                        if (responsecode === '0000') {
                            ...
                            if (roundtrip) {
                                responseSearchFlight.departure = responseSearchFlight.departure.map((val, index) => {
                                    if (val.price < flightdeparture.price) {
                                        return { ...responseSearchFlight.departure[index], price: flightdeparture.price }
                                    } else return { ...responseSearchFlight.departure[index] };
                                });
                                responseSearchFlight.return = responseSearchFlight.return.map((val, index) => {
                                    if (val.price < flightreturn.price) {
                                        return { ...responseSearchFlight.return[index], price: flightreturn.price }
                                    } else return { ...responseSearchFlight.return[index] };
                                });
                            } else responseSearchFlight.departure = responseSearchFlight.departure.map((val, index) => {
    ...
    render() {
        ...
        if (isSuccessSearchFlight && fromApproval) {
            ...
            return (<Redirect to={{ pathname: '/member/form/' + this.props.match.params.ID + '/certificate/update/' + this.props.match.params.certificateid + '/freeflight/buy', state: { priceList: responseSearchFlight, requestSearchFlight, activitydepartureselected, activityreturnselected, activitydetails, certificatedetails, memberprofile, categorycode } }} />)
        } else {
            return (
                <Row>
                    <Row>
