...
import { SalutationSelect, Button, Alert, SelectBase, SwitchButton, InputText, DatePickerBase } from '../../../../../../components/Base/BaseComponent';
import { Form, Row, Col, Divider, Typography, Spin, List, Card, Affix, Tooltip, Modal } from 'antd';
...
class App extends Component {
    ...
    async componentDidMount() {
        ...
        const { flightdeparture, flightreturn, roundtrip } = activitydetails
        ...
        if (state === undefined) {
            ...
            /*Set Field Disabled */
            let { awardcode, ticketnumber, ticketvaliditydate, redeemusers, redeemuser, paidticketnumber, awardpricingby } = (fromApproval) ? result.reqdatas : certificatedetails;
            ...
            this.props.form.setFieldsValue({ standarfee, flightnumberdeparture, flightnumberreturn, ticketnumber: ticketnumber ? ticketnumber : paidticketnumber, ticketvaliditydate, selfusage, salutationcode, name, familyname, memberid, travelertype });
            ...
            if (activitydepartureselected && !activityreturnselected) {
                const { airlinecode, bookingclasscode, compartmentcode, origin, destination, price } = activitydetails.flightdeparture || {};
                ...
                flightDeparture = (awardpricingby === 'MANUAL') ? [{ price, airlinecode, bookingclasscode, compartmentcode, origin, destination, flightdate: departuredate }] : priceList.departure.map((val, key) => {
                    return {
                        ...val,
                        paidcompartmentcode: (val.paidcompartmentcodedepart) ? val.paidcompartmentcodedepart : null,
                        paidbookingclasscode: (val.paidbookingclassdepart) ? val.paidbookingclassdepart : null
                    }
                });
            }
            /* Update Return Activity */
            else if (!activitydepartureselected && activityreturnselected) {
                const { airlinecode, bookingclasscode, compartmentcode, origin, destination, price } = activitydetails.flightreturn || {};
                ...
                flightReturn = (awardpricingby === 'MANUAL') ? [{ price, airlinecode, bookingclasscode, compartmentcode, origin, destination, flightdate: departuredate }] : priceList.departure.map((val, key) => {
                    return {
                        ...val,
                        paidcompartmentcode: (val.paidcompartmentcodedepart) ? val.paidcompartmentcodedepart : null,
                        paidbookingclasscode: (val.paidbookingclassdepart) ? val.paidbookingclassdepart : null
            ...
            /* Update Departure and Return Activity */
                ...
                flightDeparture = (awardpricingby !== 'MANUAL') ? priceList.departure.map((val, key) => {
                    return {
                        ...val,
                        paidcompartmentcode: (val.paidcompartmentcodedepart) ? val.paidcompartmentcodedepart : null,
                        paidbookingclasscode: (val.paidbookingclassdepart) ? val.paidbookingclassdepart : null
                    }
                }) : [{ price: flightdeparture.price, airlinecode: flightdeparture.airlinecode, bookingclasscode: flightdeparture.bookingclasscode, compartmentcode: flightdeparture.compartmentcode, origin: flightdeparture.origin, destination: flightdeparture.destination, flightdate: departuredate }];

                flightReturn = (awardpricingby !== 'MANUAL') ? priceList.return.map((val, key) => {
                    return {
                        ...val,
                        paidcompartmentcode: (val.paidcompartmentcodedepart) ? val.paidcompartmentcodedepart : null,
                        paidbookingclasscode: (val.paidbookingclassdepart) ? val.paidbookingclassdepart : null
                    }
                }) : [{ price: flightreturn.price, airlinecode: flightreturn.airlinecode, bookingclasscode: flightreturn.bookingclasscode, compartmentcode: flightreturn.compartmentcode, origin: flightreturn.origin, destination: flightreturn.destination, flightdate: returndate }];
            }
    ...
    getFee(awardcode, standarfee) {
        ...
        DetailRequest(url, data).then((response) => {
            ...
            if (responsecode === '0000' && result) {
                ...
                this.setState({
                    fieldvalue: { ...this.state.fieldvalue, updateunit, updatefee: updatefee ? updatefee : 0, awardtypename },
                    isLoading: false
    ...
    saveAction = (e, callbackexpiration) => {
        ...
        const callback = (input, callbackexpiration) => {
            ...
            // const { promocodedepr, promocoderetr, totalMileage } = this.state;
            ...
            } else redeemairactivityid.push(activitydetails.flightdeparture.redeemairactivityid);

            if (activitydepartureselected && activityreturnselected) {
                updatetype = 'ALL';
            } else if (activitydepartureselected && !activityreturnselected) {
                updatetype = 'DEPARTURE';
            } else if (!activitydepartureselected && activityreturnselected) updatetype = 'RETURN';
            ...
            departureActivity = {
                ...
                paidcompartmentcode: departureActivity.paidcompartmentcode,
                paidbookingclasscode: departureActivity.paidbookingclasscode,
            ...
            if (roundtrip) {
                ...
                returnActivity = {
                    ...
                    paidcompartmentcode: returnActivity.paidcompartmentcode,
                    paidbookingclasscode: returnActivity.paidbookingclasscode,
                    bookingtype: "INTERNET"
                }
            ...
            let data = fromApproval ? {
                reqdata, createdBy, createdDate, isdataactive, requestid, referenceid, memberid: reqdatas.memberid, requesttype, requeststatus, approvalby, approvaldate, remark: input.remark ? input.remark : null, reqdatas: {
                    status: reqdatas.status, url: 'redemption/transaction/v1.2/buyaward', awardcode: reqdatas.awardcode, issueddate, quantity: reqdatas.quantity, memberid: reqdatas.memberid, username, bookingcode, ticketvaliditydate, ticketnumber, freeaward: reqdatas.freeaward, return: reqdatas.return,
                ...
                awardcode, issueddate, quantity, memberid, username, bookingcode, ticketvaliditydate, ticketnumber, freeaward, return: roundtrip, totalprice,
                tierid, membershipid, updatetype, certificateid, redeemairactivityid, redeemuser, redeemairactivity, standarfee, fee
            } : {
                referenceid: null, memberid, requesttype: "UPDATE", requeststatus: "NEW", approvalby: null, approvaldate: null, remark: null, reqdatas: {
                    status, url: 'redemption/transaction/v1.2/update', awardcode, issueddate, quantity, memberid, username, bookingcode, ticketvaliditydate, ticketnumber, categorycode,
                    freeaward, return: roundtrip, totalprice, tierid, membershipid, updatetype, certificateid, redeemairactivityid, redeemuser, redeemairactivity, standarfee, fee
                }
            };
            ...
            SaveRequest(url, data).then((response) => {
                ...
                if (responsecode === "0000") {
                    ...
                    if (fromApproval) {
                        ...
                        } else this.props.history.push(`/member/form/${memberid}/certificate`);
                    }
                } else Alert.error(responsemessage);
    ...
    showConfirmation = (typeButton) => {
        ...
        if (!roundtrip && promocodedepr && activitydepartureselected) {
            this.getpromoConfirm('DEPARTURE', true, false);
        } else if (roundtrip && promocoderetr && promocodedepr && activitydepartureselected && activityreturnselected) {
            this.getpromoConfirm('DEPARTURE', true, true);
            this.getpromoConfirm('RETURN', true, true);
        } else if (roundtrip && promocoderetr && !promocodedepr && activityreturnselected) {
            this.getpromoConfirm('RETURN', false, true)
        } else if (roundtrip && !promocoderetr && promocodedepr && activitydepartureselected) {
            this.getpromoConfirm('DEPARTURE', true, false)
        } else if (roundtrip && promocoderetr && promocodedepr && activitydepartureselected) {
            this.getpromoConfirm('DEPARTURE', true, false)
    ...
    getpromo = (type, departureselected, returnselected) => {
        const { location } = this.props;
        const { state } = location;
        const { flightDeparture, flightReturn } = this.state;
        ...
        let price = (departureselected && returnselected && type === 'RETURN') ? priceList.return[0].price : departure[0].price;
    ...
    getpromoConfirm = (type, departureselected, returnselected) => {
        const { location } = this.props;
        const { state } = location;
        const { certificatedetails, activitydetails, requestSearchFlight, priceList } = state;
        const { flightDeparture, flightReturn } = this.state;
        ...
        const { awardcategory } = redeemusers;
        const { flightdeparture, flightreturn } = activitydetails;
        const { promocode, bookingclass, compartment, origin, destination, airlinecode } = (type === 'DEPARTURE') ? flightdeparture : flightreturn
        let flightnumber = (type === 'DEPARTURE') ? this.props.form.getFieldValue('flightnumberdeparture') : this.props.form.getFieldValue('flightnumberreturn')
        const { adultpassenger } = requestSearchFlight;
        const { cardnumber } = this.props
        let price = (departureselected && returnselected && type === 'RETURN') ? priceList.return[0].price : departure[0].price;
        let url = api.url.redeempromo.getpromo;
    ...
    render() {
        const formItemLayout = {
            labelCol: { xs: { span: 24 }, sm: { span: 8 }, lg: { span: 10 }, xl: { span: 9 } },
            wrapperCol: { xs: { span: 24 }, sm: { span: 10 }, lg: { span: 12 }, xl: { span: 10 } }
        };
        const { isLoading, flightDeparture, flightReturn, selectFlightDeparture, selectFlightReturn, requestSearchFlight, mileageDeparture, mileageReturn, isSuccessBuy, newcertificateid, deprpromocode, discountamountdepr, deprprice, retrpromocode, showConfirmation, awardinfo,
            discountamountretr, retrprice, discountdepr, totaldiscountdepr, totalafterdiscountdepr, promocodedepr, discounttypedepr, discountretr, totaldiscountretr, totalafterdiscountretr, promocoderetr, discounttyperetr, fieldvalue, typeButton, showsave } = this.state;
        // const { totalMileage, summaryafterdiscountdepr, summarydiscountdepr, summaryafterdiscountretr, summarydiscountretr } = this.state;
        const { adultpassenger } = requestSearchFlight;
        const { location, fromApproval, result } = this.props;
        const { state } = location;
        const { certificatedetails, activitydetails, activitydepartureselected, activityreturnselected } = state;
        // const { departure } = priceList;
        const { roundtrip } = (fromApproval) ? result.reqdatas.return : activitydetails;
        const { awardcode, awardtype, partnername, certificateid, totalprice } = (fromApproval) ? result.reqdatas : certificatedetails;
        const manualPricing = (awardinfo && (awardinfo.pricingby === 'MANUAL'));
        let summaryDepartureFlight = (flightDeparture && flightDeparture[selectFlightDeparture]) ? flightDeparture[selectFlightDeparture] : {};
        let summaryReturnFlight = (flightReturn && flightReturn[selectFlightReturn]) ? flightReturn[selectFlightReturn] : {};
        let memberid = this.props.match.params.ID;
        let passengerList = [];
        ...
        if (adultpassenger > 1) {
            for (let key = 0; key < adultpassenger - 1; key++) {
                passengerList[key] = <Col className="gutter-row" xs={24} sm={24} md={24} lg={{ span: 20, offset: 2 }} xl={{ span: 20, offset: 2 }}>
                    ...
                    <InputText form={this.props.form} labeltext="Card Number" datafield={"passenger-memberid" + key} validationrules={['pattern.number']} maxLength={16} />
                    <SalutationSelect form={this.props.form} labeltext="Salutation" datafield={"passenger-salutationcode" + key} />
                    <InputText form={this.props.form} labeltext="Name" datafield={"passenger-name" + key} validationrules={['required', 'pattern.letterspace']} maxLength={45} />
                    ...
                    <SelectBase form={this.props.form} labeltext="Traveler Type" datafield={"passenger-travelertype" + key} validationrules={['required']} options={TravelerType} />
                </Col>;
            }
        }

        let totalmileage = (manualPricing) ? ((roundtrip) ? ((pricereturn && pricedeparture) ? formatNumber((Number(pricedeparture) + Number(pricereturn)) * adultpassenger) : null) : ((pricedeparture) ? formatNumber(Number(pricedeparture) * adultpassenger) : null)) :
        ...
        let informationDetail = <div>
                ...
                <Row>
                    ...
                    <Col xs={24} sm={24} md={24} lg={12} xl={12} style={{ textAlign: 'right' }}>
                        <Tooltip title={(awardcode) ? awardcode : '-'}>
                            {(awardcode) ? (awardcode.length > 12) ? awardcode.substring(0, 12) + '...' : awardcode : '-'}
                        </Tooltip>
                    </Col>
                </Row>
            ...
            <Card title="Price Details" bordered={false} style={{ boxShadow: '0 2px 5px 0 rgba(27,27,27,.1)' }} bodyStyle={{ paddingTop: '5px', paddingBottom: '5px' }}>
                <Row>
                    <Col xs={24} sm={24} md={24} lg={18} xl={18}>
                        <Text strong style={{ display: 'block', marginBottom: 10 }}>Total Award Price Before : </Text>
                        <Text strong style={{ display: 'block' }}> Award Price After : </Text>
                        <Text style={{ display: 'block' }}> Price Details </Text>
                        <Text style={{ display: 'block' }}> Departure Flight </Text>
                        ...
                        <Text style={{ display: 'block' }}>
                            {
                                (selectFlightDeparture !== null) ?
                                    ` ${summaryDepartureFlight.origin}-${summaryDepartureFlight.destination} / ${summaryDepartureFlight.airlinecode} / ${summaryDepartureFlight.compartmentcode} / ${summaryDepartureFlight.bookingclasscode} (x ${adultpassenger} )` : "-"
                            }
                        </Text>
                        ...
                        <Text style={{ display: 'block' }}>
                            {
                                (promocodedepr) ? `Promo ${promocodedepr}` : (deprpromocode) ? `Promo ${deprpromocode}` : " "
                            }
                        </Text>
                    </Col>
                    <Col xs={24} sm={24} md={24} lg={6} xl={6} style={{ textAlign: 'right', marginBottom: 53 }}>
                        <strong>{((selectFlightDeparture !== null && activitydepartureselected) || (selectFlightReturn !== null && activityreturnselected && roundtrip)) ? formatNumber(totalprice) : (!activitydepartureselected) ? formatNumber(discountamountdepr + deprprice) : "-"}</strong>
                    </Col>
                    <Col xs={24} sm={24} md={24} lg={6} xl={6} style={{ textAlign: 'right' }}>
                        {(selectFlightDeparture !== null && activitydepartureselected) ? formatNumber(mileageDeparture * adultpassenger) :
                            (!activitydepartureselected && promocodedepr && discountamountdepr && deprprice) ? formatNumber(discountamountdepr + deprprice) :
                                (!activitydepartureselected && !promocodedepr && deprprice) ? formatNumber(deprprice) : "-"}
                    </Col>
                    <Col xs={24} sm={24} md={24} lg={6} xl={6} style={{ textAlign: 'right', marginTop: 63 }}>
                        {(totaldiscountdepr && activitydepartureselected) ? `- ${formatNumber(totaldiscountdepr * adultpassenger)}` :
                            (!totaldiscountdepr && activitydepartureselected) ? "" :
                                (!activitydepartureselected && activityreturnselected && discountamountdepr && promocodedepr) ? `- ${formatNumber(discountamountdepr * adultpassenger)}` :
                    ...
                    <Col xs={24} sm={24} md={24} lg={18} xl={18} style={{ display: 'block', marginLeft: 30 }}>
                        <Text style={{ display: 'block' }}>
                            {(discountdepr && discounttypedepr === "PERCENTAGE" && totaldiscountdepr) ? `(Disc ${discountdepr}%) @${formatNumber(totaldiscountdepr)} x ${adultpassenger}` :
                                (!discountdepr && deprpromocode) ? `Disc @${formatNumber(discountamountdepr)} x ${adultpassenger}` :
                    ...
                    (roundtrip) ?
                        <Row style={{ marginTop: '5px' }}>
                            <Col xs={24} sm={24} md={24} lg={18} xl={18}>
                                ...
                                <Text style={{ display: 'block' }}>
                                    {
                                        (promocoderetr) ? `Promo ${promocoderetr}` : (retrpromocode) ? `Promo ${retrpromocode}` : " "
                                    }
                                </Text>
                            </Col>
                            <Col xs={24} sm={24} md={24} lg={6} xl={6} style={{ textAlign: 'right' }}>
                                {(selectFlightReturn !== null && activityreturnselected) ? formatNumber(mileageReturn * adultpassenger) :
                                    (!activityreturnselected && promocoderetr && discountamountretr && retrprice) ? formatNumber(discountamountretr + retrprice) :
                                        (!activityreturnselected && !promocoderetr && retrprice) ? formatNumber(retrprice) : "-"}
                            </Col>
                            <Col xs={24} sm={24} md={24} lg={6} xl={6} style={{ textAlign: 'right', marginTop: 63 }}>
                                {(totaldiscountretr && activityreturnselected) ? `- ${formatNumber(totaldiscountretr * adultpassenger)}` :
                                    (!totaldiscountretr && activityreturnselected) ? "" :
                                        (activitydepartureselected && !activityreturnselected && discountamountretr && promocoderetr) ? `- ${formatNumber(discountamountretr * adultpassenger)}` :
                ...
                <Row style={{ paddingTop: '16px', borderTop: '1px solid #e8e8e8', marginTop: '12px' }}>
                    <Col xs={24} sm={24} md={24} lg={18} xl={18}>Total Mileage</Col>
                    <Col xs={24} sm={24} md={24} lg={6} xl={6} style={{ textAlign: 'right' }}>{totalmileage}</Col>
                </Row>
            </Card>
            {(fromApproval) ? <Row>
        ...
        if (isSuccessBuy) {
            return (<Redirect to={{ pathname: `/member/form/${memberid}/certificate/view/${newcertificateid}` }} />)
        } else {
            return (
                <Row>
                    {
                        (showConfirmation) ? <ConfirmationPage isLoading={isLoading} certificateid={certificateid} memberid={memberid} departureFlight={fieldvalue.summaryDepartureFlight} returnFligth={fieldvalue.summaryReturnFlight} roundtrip={roundtrip} visible={showConfirmation} handleClose={this.handleCancel} totalmileage={totalmileage}
                    ...
                    <TicketNumber visible={this.state.visible} handleCancel={this.handleCancel} />
                    {(fromApproval) ? '' : <Row>
                        <Title level={4}><Button url={`/member/form/${memberid}/certificate`} shape="circle" icon="left" /> Certificate Update</Title>
                        <Divider />
                    </Row >}
                    ...
                    <Spin spinning={this.state.isLoading}>
                        <Form {...formItemLayout} onSubmit={this.showConfirmation}>
                            <Row>
                                <Col className="gutter-row" xs={24} sm={24} md={24} lg={15} xl={16}>
                                    <Row>
                                        ...
                                        <Col className="gutter-row" xs={24} sm={24} md={24} lg={24} xl={24}>
                                            ...
                                            <Row className="pricelist-flight-header">
                                                ...
                                                <Col xs={24} lg={(manualPricing) ? 6 : 5}>Origin - Destination</Col>
                                                {(manualPricing) ? <Col xs={24} lg={10}>Compartment / Booking Class</Col> : <Col xs={24} lg={6}>Compartment /<br />Booking Class</Col>}
                                                {(manualPricing) ? null : <Col xs={24} lg={5}>Price</Col>}
                                            </Row >
                                            <List
                                                ...
                                                renderItem={(item, key) => (
                                                    <Row className="pricelist-flight">
                                                        <Col xs={24} lg={4}>{(item.flightdate) ? moment(item.flightdate).format("DD/MM/YYYY") : '-'}</Col>
                                                        <Col xs={24} lg={4}>{(item.airlinecode) ? item.airlinecode : '-'}</Col>
                                                        <Col xs={24} lg={(manualPricing) ? 6 : 5}>{(item.origin) ? item.origin : '-'} - {(item.destination) ? item.destination : '-'}</Col>
                                                        <Col xs={24} lg={(manualPricing) ? 10 : 6}>
                                                            {(item.compartmentcode) ? item.compartmentcode : '-'}&nbsp;/&nbsp;
                                                            {(item.bookingclasscode) ? item.bookingclasscode : '-'}
                                                        </Col>
                                                        {
                                                            (manualPricing) ? null : <Col xs={24} lg={4} xl={5}>
                                                                {(totalafterdiscountdepr) ? formatNumber(totalafterdiscountdepr) : (item.price) ? formatNumber(item.price) : '-'}
                                                            </Col>
                                                        }
                                                        {
                                                            (item.status === 'VOUCHER_REDEEM') ? <Col xs={24} sm={24} md={24} lg={4} xl={4}>
                                            ...
                                            <Row>
                                                <Col className="gutter-row" xs={24} sm={24} md={24} lg={24} xl={24}>
                                                    ...
                                                    <List
                                                        ...
                                                        renderItem={(item, key) => (
                                                            <Row className="pricelist-flight">
                                                                <Col xs={24} lg={(item.status === 'VOUCHER_REDEEM') ? 4 : 4}>
                                                                    {(item.flightdate) ? moment(item.flightdate).format("DD/MM/YYYY") : '-'}</Col>
                                                                <Col xs={24} lg={(item.status === 'VOUCHER_REDEEM') ? 4 : 4}>
                                                                    {(item.airlinecode) ? item.airlinecode : '-'}
                                                                </Col>
                                                                <Col xs={24} lg={(item.status === 'VOUCHER_REDEEM') ? 4 : 5}>
                                                                    {(item.origin) ? item.origin : '-'} - {(item.destination) ? item.destination : '-'}
                                                                </Col>
                                                                <Col xs={24} lg={(item.status === 'VOUCHER_REDEEM') ? 4 : 6}>
                                                                    {(item.compartmentcode) ? item.compartmentcode : '-'}&nbsp;/&nbsp;
                                                                    {(item.bookingclasscode) ? item.bookingclasscode : '-'}
                                                                </Col>
                                                                <Col xs={24} lg={(item.status === 'VOUCHER_REDEEM') ? 4 : 5}>
                                    ...
                                    <Row className={((!roundtrip && selectFlightDeparture !== null) || (roundtrip && selectFlightDeparture !== null && selectFlightReturn !== null)) ? '' : 'hidden'}>
                                        ...
                                        <Col className="gutter-row" xs={24} sm={24} md={24} lg={{ span: 23, offset: 1 }} xl={{ span: 20, offset: 2 }}>
                                            ...
                                            <InputText form={this.props.form} labeltext="Departure Flight Number" datafield="flightnumberdeparture" validationrules={['required', 'pattern.number']} maxLength={4} disabled={!activitydepartureselected} />
                                            {
                                                (roundtrip) ? <span>
                                                    ...
                                                    <InputText form={this.props.form} labeltext="Return Flight Number" datafield="flightnumberreturn" validationrules={['required', 'pattern.number']} maxLength={4} disabled={!activityreturnselected} />
                                                </span> : null
                                            }
                                        </Col >
                                    </Row >
                                    <Row className={((!roundtrip && selectFlightDeparture !== null) || (roundtrip && selectFlightDeparture !== null && selectFlightReturn !== null)) ? '' : 'hidden'}>
                                        ...
                                        <Col className="gutter-row" xs={24} sm={24} md={24} lg={{ span: 21, offset: 3 }} xl={{ span: 20, offset: 2 }}>
                                            <Divider>Passenger #1</Divider>
                                            <SwitchButton form={this.props.form} labeltext="Self Usage" datafield="selfusage" disabled={true} />
                                            <InputText form={this.props.form} labeltext="Card Number" datafield="memberid" disabled={true} validationrules={['pattern.number']} maxLength={16} />
                                            <SalutationSelect ref={(e) => { this.componentSalutationSelect = e }} form={this.props.form} labeltext="Salutation" datafield="salutationcode" disabled={true} />
                                            <InputText form={this.props.form} labeltext="Name" datafield="name" validationrules={['required', 'pattern.letterspace']} maxLength={45} disabled={true} />
                                            <InputText wrapperCol={{ span: 10 }} labelCol={{ span: 9 }} form={this.props.form} labeltext="Family Name" datafield="familyname" validationrules={['pattern.letter']} maxLength={45} disabled={true} />
                                            <SelectBase form={this.props.form} labeltext="Traveler Type" datafield="travelertype" validationrules={['required']} options={TravelerType} disabled={true} />
                                        </Col>
                                        {passengerList}
                                    </Row>
                                    <Row className={((!roundtrip && selectFlightDeparture !== null) || (roundtrip && selectFlightDeparture !== null && selectFlightReturn !== null)) ? '' : 'hidden'}>
                                        ...
                                        <Col className="gutter-row" xs={24} sm={24} md={24} lg={{ span: 23, offset: 1 }} xl={{ span: 20, offset: 2 }}>
                                            ...
                                            <InputText form={this.props.form} labeltext="Booking Code" datafield="bookingcode" validationrules={['required', 'pattern.alphanumeric']} maxLength={6} />
                                            <DatePickerBase form={this.props.form} labeltext="Ticket Validity Date" datafield="ticketvaliditydate" minDate={moment(new Date())} disabled={true} />
                                            <InputText form={this.props.form} labeltext="Paid Ticket Number" datafield="ticketnumber" validationrules={['pattern.alphanumeric']} maxLength={13} disabled={true} />

                                            <SwitchButton form={this.props.form} labeltext="Standar Fee" datafield="standarfee" onChange={this.handleStandardFee} disabled={true} />
                                            <InputText form={this.props.form} labeltext="Fee" datafield="fee" className={(standarfee) ? 'hidden' : ''} validationrules={['required', 'pattern.number']} maxLength={6} disabled={standarfee} />
                                            {
                                                (standarfee) ? <Form.Item label="Fee"><span className="ant-form-text">{fee}</span></Form.Item> : null
                                            }
                                        </Col>
                                    </Row>
                                </Col>
                                <Col className="gutter-row" xs={24} sm={24} md={24} lg={9} xl={8} style={{ padding: '0px 0px 0px 10px' }}>
                                    {(window.innerWidth > 970) ? <Affix>{informationDetail}</Affix> : informationDetail}
                                </Col>
                            </Row>
                        </Form >
                    </Spin >
