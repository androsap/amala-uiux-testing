...
class App extends React.Component {
    ...
    render() {
        const { roundtrip, departureFlight, returnFligth, certificateid, memberid, typeButton } = this.props;
        const { location, discountamountdepr, discountamountretr, mileageDeparture, mileageReturn } = this.props || {};
        const { state } = location;
        const { activitydetails } = state;
        const { flightdeparture, flightreturn } = activitydetails;
        let pricedepr = mileageDeparture ? mileageDeparture : null
        let priceretr = mileageReturn ? mileageReturn : null
        let discountdepr = discountamountdepr ? discountamountdepr : null
        let discountretr = discountamountretr ? discountamountretr : null
                ...
                >
                    {
                        (formrender) ?
                            <Spin spinning={isLoading}>
                                <Card title="Previous Change Schedule Information" bordered={false} style={{ marginBottom: 10 }} className="card-shadow">
                                ...
                                <Card title="New Change Schedule Information" bordered={false} style={{ marginBottom: 10 }} className="card-shadow">
                                    <Row className="pricelist-flight-header">
                                        <Col xs={24} sm={24} md={24} lg={4} xl={4}> Flight </Col>
                                        <Col xs={24} sm={24} md={24} lg={5} xl={5}> Activity Date </Col>
                                        <Col xs={24} sm={24} md={24} lg={5} xl={5}> Airline / Flight Number </Col>
                                ...
                                <Card title="New Miles Information" bordered={false} style={{ marginBottom: 20 }} className="card-shadow">
                                    <Row gutter={24}>
                                        {/* <Col className="gutter-row" span={12}>
                                            ...
                                            <Col xs={24} xl={12}>: {(expiredawardmiles !== null) ? expiredawardmiles : '-'}</Col>
                                        </Col> */}
                                        <Col className="gutter-row" span={12}>
                                            <Col xs={24} xl={12}><label>Award Price After</label></Col>

                                            <Col xs={24} xl={12}>: {(awardmiles !== null) ? `- ${formatNumber((pricedepr - discountdepr) + (priceretr - discountretr))}` : '-'} Miles</Col>
                                        </Col>
                                    </Row>
                                </Card>
