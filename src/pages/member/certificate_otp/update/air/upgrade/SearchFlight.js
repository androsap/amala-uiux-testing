...
class App extends Component {
            ...
            if (!err) {
                ...
                if (roundtrip) {
                    ...
                    if (responseSearchFlight.length > 1) {
                        ...
                        response.departure = (responseSearchFlight[0] && responseSearchFlight[0]['departure']) ? responseSearchFlight[0]['departure'] : [];
                        response.return = (responseSearchFlight[1] && responseSearchFlight[1]['departure']) ? responseSearchFlight[1]['departure'] : [];
                    ...
                    if (activitydepartureselected || (result !== undefined && result.reqdatas.updatetype === 'DEPARTURE')) {
                        departuredate = (input.departuredate) ? moment(input.departuredate).format("YYYY-MM-DD") : null;
                    } else if (activityreturnselected || (result !== undefined && result.reqdatas.updatetype === 'RETURN')) {
    ...
    render() {
        ...
        const { location, fromApproval, result, countdown, validate } = this.props;
        const { state } = location || {};
        ...
        if (validate) {
            ...
            if (isSuccessSearchFlight && fromApproval) {
                ...
                return (<Redirect to={{ pathname: '/member/form/' + this.props.match.params.ID + 'certificateotp/update/' + this.props.match.params.certificateid + '/upgrade/buy', state: { priceList: responseSearchFlight, requestSearchFlight: requestSearchFlight, activitydepartureselected, activityreturnselected, activitydetails, certificatedetails, memberprofile, categorycode } }} />)
            } else {
                return (
                    <Row>
                        <Row>
                                    ...
                                    <Col className="gutter-row" xs={24} sm={24} md={24} lg={{ span: 14, offset: 3 }} xl={{ span: 14, offset: 3 }}>
...

                                        {
                                            (roundtrip) ?
                                                ...
                                                :
                                                (activitydepartureselected || (result !== undefined && result.reqdatas.updatetype === 'DEPARTURE')) ?
                                                    <span>
                                                        <Form.Item label="Old Departure Date">
                                                            <span className="ant-form-text">{(flightdeparture && flightdeparture.activitydate) ? moment(flightdeparture.activitydate).format("DD/MM/YYYY") : '-'}</span>
                                                        </Form.Item>
                                                    ...
                                                    (activityreturnselected || (result !== undefined && result.reqdatas.updatetype === 'RETURN')) ?
                                                        <span>
                                                            <Form.Item label="Old Return Date">
                                                                <span className="ant-form-text">{(flightreturn && flightreturn.activitydate) ? moment(flightreturn.activitydate).format("DD/MM/YYYY") : '-'}</span>
                                                            </Form.Item>
