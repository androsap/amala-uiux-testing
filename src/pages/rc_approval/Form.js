...
class Layout extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            loading: false,
            errors: {},
            titlepage: 'Approval Retro Claim Request Detail',
            actionspage: 'create',
            responseCode: '0',
            responseMessage: '',
    ...
    checkPermission() {
        ...
        if (id) {
            let titlepage = 'Edit Approval Retro Claim Request';
            ...
            this.setState({ titlepage, actionspage, specialfielddisabled, compartmentcodedisabled, subclasscodedisabled, formtype, retrofrom });
            this.getDetail(id, retrofrom);
        } else {
    ...
    getDetail(retroclaimid, retrofrom) {
        ...
        RetrieveRequest(url, paging, column, criteria, sort).then((response) => {
            ...
            if (status.responsecode.substring(0, 1) === '0' && result) {
                if (result.length !== 0) {
                    ...
                    this.refs.cardnumber.value = cardnumber;
                    ...
                    let subclasscode = result[0].cabinclasscode ? result[0].cabinclasscode : null;
                    subclasscode = (this.state.formtype !== 'approval') ? subclasscode : null;

                    let compartmentcodedisabled = airlinecode ? false : true;
                    let subclasscodedisabled = compartmentcode ? false : true;
                    this.setState({
                        cardnumber, airlinecode, airlinename, subclasscode, departuredate, origin, destination,
            ...
            if (requestData) {
                requestData.then((response) => {
                    ...
                    if (responsecode.substring(0, 1) === '0') {
                        ...
                        this.props.history.push(`/${this.props.match.url.split('/')[1]}/${retrofrom}/form/${retroclaimid}`);
    ...
    render() {
        ...
        const { cardnumber, retroclaimid, airlinecode, optionsAirline, compartmentcode, optionsCompartment, optionsSubclass, subclasscode, departuredate, optionsOrigin, optionsDestination, origin, destination } = this.state;
        ...
        if (formrender) {
            ...
            return (
                <div className="container-fluid">
                    <Breadcrumb path="Accrual Data Management / Retro Claim / Approval Retro Claim" />
                            ...
                            <div className="col-sm-12">
                                <form className="clearfix position-relative" onSubmit={(e) => this.saveAction(e)} autoComplete="off">
                                                    ...
                                                    <div className="col-sm-8 input-group">
                                                        ...
                                                        <CopyToClipboard text={cardnumber}>
                                                            <div className="input-group-prepend">
                                                                <span className="input-group-text" id="basic-addon1" style={{ cursor: 'pointer' }}>Copy</span>
                                                            </div>
                                                        </CopyToClipboard>
                                    ...
                                    <div className="box-footer text-center">
                                        {(formtype === 'approval') ? <Button type="submit" label="Approve" className="btn btn-primary normal mr-2" menucode={menucode} prefixmenuname={prefixmenuname} actioncode="APPROVE" onClick={(e) => this.saveAction(e, 'approve')}></Button> : ''}
                                        {(formtype === 'approval') ? <Button type="submit" label="Reject" className="btn btn-danger normal mr-2" menucode={menucode} prefixmenuname={prefixmenuname} actioncode="APPROVE" onClick={(e) => this.saveAction(e, 'reject')}></Button> : ''}
                                        {(formtype === 'manual-verification') ? <Button type="submit" label="Manual Verification" className="btn btn-primary normal mr-2" menucode={menucode} prefixmenuname={prefixmenuname} actioncode="APPROVE" onClick={(e) => this.saveAction(e, 'verify')}></Button> : ''}
                                        <Link to={"/retro-claim-approval/" + retrofrom} className="btn btn-outline-dark normal">Back</Link>
                                    </div>
                                </form>
                            </div>
                        </div>
