...
class App extends React.Component {
    componentDidMount() {
        document.title = "Manage Enroll Bonus | Loyalty Management System";
    }

    deleteData(tierenrollbonusid) {
        let url = api.url.enrollbonus.delete;
        let data = { tierenrollbonusid };
    ...
    render() {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: (value, row, index) => {
                        return (
                            <span>
                                <Button url={'/enroll-bonus/form/' + row.tierenrollbonusid} size="small" label="Edit" menucode={menucode} prefixmenuname={prefixmenuname} actioncode="UPDATE" />
                                <Button htmlType="button" size="small" label="Delete" type="danger" menucode={menucode} prefixmenuname={prefixmenuname} actioncode="DELETE" onClick={() => this.deleteData(row.tierenrollbonusid)} />
                            </span>
                        )
                    }
                },
        ...
        return (
            <React.Fragment>
                <Row>
                    <Col xs={24} xl={22}>
                        <Title level={3}>Manage Enroll Bonus</Title>
                    </Col>
                    <Col xs={24} xl={2}>
                        <Button type="primary" url={'/enroll-bonus/form/'} size="default" label="Add New" menucode={menucode} prefixmenuname={prefixmenuname} actioncode="CREATE" />
                    </Col>
                    <Divider />
                </Row>
