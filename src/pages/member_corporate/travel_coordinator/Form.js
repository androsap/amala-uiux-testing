...
class App extends Component {
                ...
                if (result.length !== 0) {
                    ...
                    if (travelcordinatorid) {
                        this.props.form.setFieldsValue({
                            ['staff[0][name]']: name,
                            ['staff[0][birthdate]']: moment(birthdate),
                            ['staff[0][cardnumber]']: cardnumber,
                            ['staff[0][idcardnumber]']: idcardnumber,
                            ['staff[0][email]']: email,
                            ['staff[0][username]']: username,
                            ['staff[0][phonenumber]']: phonenumber,
                        });
                    }
    ...
    saveAction = (e) => {
        ...
        this.props.form.validateFieldsAndScroll((err, input) => {
            if (!err) {
                ...
                let travelcoordinatortype = (actionspage === 'create') ? 'STAFF' : input.travelcoordinatortype;
                ...
                let travelcordinator = travelcordinatorstaff.filter(element => { return element !== null; })[0];
                // let reqdatas = {
    ...
    render() {
        const { actionspage, specialfielddisabled, travelcoordinatortype, staffidcardnumberundisabled } = this.state;
                    ...
                    <Form {...formItemLayout} onSubmit={this.saveAction}>
                        <Row gutter={24}>
                            <Col className="gutter-row" xs={24} sm={24} md={24} lg={{ span: 18 }} xl={{ span: 18 }}>
                                <SelectBase form={this.props.form} labeltext="Travel Coordinator Type" datafield="travelcoordinatortype" options={CorporateRole} defaultValue={(actionspage === 'create') ? 'STAFF' : travelcoordinatortype} disabled />
                                <InputText form={this.props.form} labeltext="Card Number" datafield={`staff[0][cardnumber]`} validationrules={['pattern.number', 'required']} maxLength={20} onBlur={(e) => this.handleAutoFill(e, 'staff', 0)} disabled={specialfielddisabled} />
                                <InputText form={this.props.form} labeltext="Name" datafield={`staff[0][name]`} validationrules={['pattern.name', 'required']} maxLength={45} disabled={true} />
                                <InputText form={this.props.form} labeltext="Username" datafield={`staff[0][username]`} validationrules={['required']} maxLength={45} disabled={true} />
                                <InputText form={this.props.form} labeltext="ID Card Number" datafield={`staff[0][idcardnumber]`} validationrules={['pattern.number', 'required']} maxLength={20} disabled={disabled} />
                                <InputText form={this.props.form} labeltext="Phone Number" datafield={`staff[0][phonenumber]`} validationrules={['pattern.phonenumber', 'required']} maxLength={20} disabled={true} />
                                {
                                    (actionspage === 'create') ?
                                        <InputText form={this.props.form} labeltext="Email" datafield={`staff[0][email]`} validationrules={['pattern.email', 'required']} maxLength={255} disabled={true} />
                                        :
                                        <InputText form={this.props.form} labeltext="Email" datafield={`staff[0][email]`} validationrules={['pattern.email', 'required']} maxLength={255} disabled={false} />
                                }
                                <DatePickerBase form={this.props.form} labeltext="Birth Date" datafield={`staff[0][birthdate]`} validationrules={['required']} maxDate={moment()} disabled={true} />
                                <SelectBase form={this.props.form} labeltext="Corporate Role" datafield="rolecode" validationrules={(specialfielddisabled ? [] : ['required'])} options={CorporateRole} disabled={true} defaultValue={(actionspage === 'create') ? 'STAFF' : travelcoordinatortype} />
                            </Col>
                        </Row>
